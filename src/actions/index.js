...
export const addNote = (notebook, data) => {
  return dispatch => {
    return fetch(`http://13.92.34.183/notebook/${notebook}/note`, {
      method: 'POST',
      headers: {
...
export const deleteNote = (notebook, id) => {
  return dispatch => {
    return fetch(`http://13.92.34.183/notebook/${notebook}/note/${id}`, {
      method: 'DELETE'
    })
      .then(response => response.json())
      .then(json => dispatch(getNotes(notebook)));
...
function receiveCategories(json) {
  return {
    ...
    categories: json.map(cat => new Category(cat.id, cat.category))
  };
}
